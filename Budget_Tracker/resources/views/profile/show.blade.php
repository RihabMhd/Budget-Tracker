        ...
        <div class="profile-hero">
            <div class="hero-avatar" style="padding:0;overflow:hidden;position:relative;">
                @if ($user->profile_photo)
                    <img src="{{ asset($user->profile_photo) }}" alt="Avatar" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
                @else
                    <span style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;">{{ strtoupper(substr($user->username, 0, 2)) }}</span>
                @endif
            </div>
            <div class="hero-info">
        ...
        <div class="two-col">
            ...
            <div style="display:flex;flex-direction:column;gap:20px;">
                ...
                <div class="panel">
                    ...
                    <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;">
                        <div style="width:64px;height:64px;border-radius:50%;overflow:hidden;flex-shrink:0;background:rgba(251,207,151,0.15);border:2px solid rgba(251,207,151,0.3);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:600;color:#FBCF97;">
                            @if ($user->profile_photo)
                                <img src="{{ asset($user->profile_photo) }}" style="width:100%;height:100%;object-fit:cover;">
                            @else
                ...
                <div class="panel">
                    ...
                    @if ($budget > 0)
                        <div style="margin-bottom:16px;">
                            <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px;">
                                ...
                                <span style="color:#ccc;font-weight:600;">{{ number_format($monthlySpent, 2) }} / {{ number_format($budget, 2) }} DH</span>
                            </div>
                            <div style="height:8px;background:#e5d9c8;border-radius:99px;overflow:hidden;margin-bottom:6px;">
                                <div style="height:100%;width:{{ $spentPct }}%;background:{{ $barColor }};border-radius:99px;"></div>
                            </div>
                    ...
                    @else
                        <p style="font-size:13px;color:#888;margin-bottom:14px;">No budget set yet. Add your monthly salary below.</p>
                    @endif
                    <form method="POST" action="{{ route('profile.budget') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="monthly_budget" step="0.01" min="0"
                                   ...
                                   placeholder="e.g. 6000" required>
                            @error('monthly_budget') <div class="form-error">{{ $message }}</div> @enderror
                        </div>
                ...
                <div class="panel">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="username" class="form-input {{ $errors->has('username') ? 'danger' : '' }}" value="{{ old('username', $user->username) }}" required>
                            @error('username') <div class="form-error">{{ $message }}</div> @enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">Email Address</label>
                            <input type="email" name="email" class="form-input {{ $errors->has('email') ? 'danger' : '' }}" value="{{ old('email', $user->email) }}" required>
            ...
            <div style="display:flex;flex-direction:column;gap:16px;">
                ...
                @php
                    ...
                    $progressPct = $nextThreshold > $prevThreshold
                        ? min(100, round((($user->points - $prevThreshold) / ($nextThreshold - $prevThreshold)) * 100))
                        : 100;
                @endphp

                <div class="panel" style="padding:22px; position:relative; overflow:hidden;">
                    <div style="position:absolute;top:-30px;right:-30px;width:130px;height:130px;border-radius:50%;background:rgba(251,207,151,0.06);pointer-events:none;"></div>
                    <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#888;margin-bottom:4px;">🚀 Rank Progression</div>

                    @if ($currentRank)
                        <div style="display:flex;align-items:center;gap:18px;margin-top:14px;">
                            <div style="width:90px;height:90px;flex-shrink:0;border-radius:50%;background:rgba(251,207,151,0.08);border:1px solid rgba(251,207,151,0.2);display:flex;align-items:center;justify-content:center;">
                                <img src="{{ asset($currentRank->image_path) }}" alt="{{ $currentRank->title }}" style="width:62px;height:62px;object-fit:contain;">
                            </div>
                            <div style="flex:1;">
                                <div style="font-size:18px;font-weight:600;color:#FBCF97;margin-bottom:2px;">{{ $currentRank->title }}</div>
                                <div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">Current Mastery Level</div>
                                @if ($nextRank)
                                    <div style="display:flex;justify-content:space-between;font-size:11px;color:#888;margin-bottom:5px;">
                                        ...
                                        <span>{{ $nextRank->title }} — {{ number_format($nextRank->points_required) }} pts</span>
                                    </div>
                                    <div style="height:8px;background:#e5d9c8;border-radius:99px;overflow:hidden;">
                                        <div style="height:100%;width:{{ $progressPct }}%;background:#BA7517;border-radius:99px;"></div>
                                    </div>
                                    <div style="font-size:11px;color:#888;margin-top:5px;">{{ number_format($nextRank->points_required - $user->points) }} pts to next rank</div>
                                @else
                ...
                <div class="panel" style="padding:14px 20px;display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        <div style="font-size:22px;font-weight:600;color:#FBCF97;">{{ number_format($user->points) }}</div>
                    ...
                    @if ($nextRank)
                        <div style="font-size:12px;color:#888;margin-left:auto;">
                            <span style="color:#BA7517;font-weight:600;">{{ number_format($nextRank->points_required - $user->points) }} pts</span> until {{ $nextRank->title }}
                        </div>
                    @endif
                </div>
                ...
                <div class="panel" style="padding:22px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                        <div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:500;color:#ccc;text-transform:uppercase;letter-spacing:1px;">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FBCF97" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <span style="font-size:11px;color:#888;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:99px;padding:2px 10px;">{{ $badges->count() }} earned</span>
                    </div>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(78px,1fr));gap:10px;">
                        @foreach ($allRanks as $rank)
                            ...
                            <div title="{{ $rank->title }}{{ !$earned ? ' — ' . number_format($rank->points_required) . ' pts required' : '' }}"
                                ...
                                <img src="{{ asset($rank->image_path) }}" alt="{{ $rank->title }}" style="width:40px;height:40px;object-fit:contain;">
                                <div style="font-size:10px;color:{{ $earned ? '#FBCF97' : '#888' }};text-align:center;line-height:1.3;">{{ $rank->title }}</div>
                            </div>
                        @endforeach
                    </div>

                    @if ($badges->isEmpty())
                        <p style="font-size:13px;color:#888;text-align:center;padding:16px 0;">No badges yet — keep going! 💪</p>
                    @endif
                </div>
