                ...
                <div class="filter-group">
                    ...
                    <div class="cat-select-wrap" id="filter-cat-wrap">
                        <div class="cat-select-trigger" id="filter-cat-trigger" onclick="toggleCatDropdown('filter')">
                            ...
                            <span
                                id="filter-cat-label">{{ collect($categories)->firstWhere('id', request('category_id'))?->name ?? 'All categories' }}</span>
                            <svg viewBox="0 0 24 24"
                        ...
                        <div class="cat-select-dropdown hidden" id="filter-cat-dropdown">
                            <div class="cat-option"
                                onclick="selectCat('filter', '', 'All categories', 'transparent', true)">
                            ...
                            @foreach ($categories as $cat)
                                <div class="cat-option"
                                    onclick="selectCat('filter', '{{ $cat->id }}', '{{ $cat->name }}', '{{ $cat->color ?? '#FBCF97' }}', false)">
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    <div class="tx-icon-cell"
                                        style="background:{{ $tx->type === 'Income' ? '#d1fae5' : '#fff3e0' }};">
                                        {{ $tx->type === 'Income' ? '💰' : '💸' }}
                                    </div>
                                    <div>
                            ...
                            <td>
                                <div style="display:flex;gap:6px;">
                                    <a href="{{ route('transactions.edit', $tx) }}" class="action-btn"><svg
                                    ...
                                    <form method="POST" action="{{ route('transactions.destroy', $tx) }}"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit" class="action-btn danger"><svg
                                                xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="text-align:center;padding:48px;color:#ccc;">
                                ...
                                <div style="font-size:13px;color:#ddd;margin-top:4px;">Add your first transaction using the
                                    button above.</div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="form-group">
                    ...
                    <div class="cat-select-wrap" id="modal-cat-wrap">
                        <div class="cat-select-trigger" id="modal-cat-trigger" onclick="toggleCatDropdown('modal')">
                            ...
                            <span
                                id="modal-cat-label">{{ collect($categories)->firstWhere('id', old('category_id'))?->name ?? 'Select a category' }}</span>
                            <svg viewBox="0 0 24 24"
                                style="width:12px;height:12px;fill:none;stroke:#aaa;stroke-width:2.5;stroke-linecap:round;margin-left:auto;">
                                <polyline points="6,9 12,15 18,9" />
                            </svg>
                        ...
                        <div class="cat-select-dropdown hidden" id="modal-cat-dropdown">
                            @foreach ($categories as $cat)
                                <div class="cat-option"
                                    onclick="selectCat('modal', '{{ $cat->id }}', '{{ $cat->name }}', '{{ $cat->color ?? '#FBCF97' }}', false)">
                                    <span class="cat-option-dot"
