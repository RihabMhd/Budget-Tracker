...
@push('styles')
    <style>
        ...
        * {
        ...
        .stat-card.accent-peach .stat-change {
            color: #8a5a1a;
        }

        .stat-card.accent-peach .stat-value {
            color: #1C1C1E;
        }

        .stat-card.accent-green .stat-label,
        .stat-card.accent-green .stat-change {
            color: #0a5530;
        }
        ...
        .stat-card.accent-peach .stat-icon {
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div>
                <div class="topbar-title">
                    ...
                    @endphp
                    {{ $greeting }}, {{ Auth::user()->name ?? Auth::user()->username }}
                </div>
            ...
            <div style="display:flex;align-items:center;gap:12px;">
                <button class="month-selector">
                    <svg viewBox="0 0 24 24"
                        style="width:15px;height:15px;fill:none;stroke:#888;stroke-width:2;stroke-linecap:round">
                    ...
                    {{ now()->format('F Y') }}
                    <svg viewBox="0 0 24 24"
                        style="width:13px;height:13px;fill:none;stroke:#888;stroke-width:2.5;stroke-linecap:round">
        ...
        @if (Auth::user()->current_streak > 0)
            <div class="streak-row">
                ...
                <span class="streak-text">You're on a <span>{{ Auth::user()->current_streak }}-day streak</span> — keep
                    logging your expenses!</span>
            </div>
        @endif
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card accent-peach">
                ...
                <div class="stat-value" style="color:{{ $totalBalance < 0 ? '#e05c5c' : '#1C1C1E' }};">
                    ${{ number_format($totalBalance, 2) }}
                </div>
                <div class="stat-change" style="color:#8a5a1a;">All-time net balance</div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value" style="color:{{ $remainingWallet < 0 ? '#e05c5c' : '#1a1a1a' }};">
                ...
                <div style="margin-top:10px;">
                    <div style="display:flex;justify-content:space-between;font-size:11px;color:#aaa;margin-bottom:5px;">
                        <span>{{ round($spentPercentage) }}% of budget spent</span>
                        <span>${{ number_format($startingAllowance, 2) }} budget</span>
                    </div>
                    <div style="height:5px;background:#f0ece4;border-radius:99px;overflow:hidden;">
                        <div style="height:100%;border-radius:99px;width:{{ min(100, $spentPercentage) }}%;background:{{ $spentPercentage > 90 ? '#e05c5c' : '#2EB872' }};transition:width 0.8s ease;"></div>
                    </div>
                </div>
                <div class="stat-bg-decoration"></div>
            </div>
            ...
            <div class="stat-card accent-green">
                ...
                <div class="stat-change up" style="color:#a8f0cc;">This month</div>
                <div class="stat-bg-decoration"></div>
            </div>
            ...
            <div class="panel">
                        ...
                        <div class="bar-group">
                            <div class="bar-wrap">
                                <div class="bar income"
                                    style="height:{{ $incH }}px; opacity:{{ $isLast ? 1 : 0.55 }};"></div>
                                <div class="bar expense"
                                    style="height:{{ $expH }}px; opacity:{{ $isLast ? 1 : 0.55 }};"></div>
                            </div>
                            <div class="bar-label" style="{{ $isLast ? 'color:#1a1a1a;font-weight:700;' : '' }}">
                                {{ $month }}</div>
                        </div>
                    @endforeach
                </div>
                <div class="chart-legend">
                    <div class="legend-item">
                        <div class="legend-dot" style="background:#2EB872;margin-top:4px;"></div> Income
                    </div>
                    <div class="legend-item">
            ...
            <div style="display:flex;flex-direction:column;gap:18px;">
                        ...
                        <div class="goal-amounts">
                            ...
                            <div style="text-align:right;">
                                ...
                                <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:#888;">
                                    ${{ number_format($goalTarget) }}</div>
                            </div>
                        </div>
                        <div class="goal-progress-wrap">
                            <div class="goal-progress-fill" style="width:{{ $goalPct }}%;"></div>
                        </div>
                        <div class="goal-pct"><strong>{{ $goalPct }}% complete</strong> —
                            ${{ number_format($goalTarget - $goalSaved) }} to go</div>
                    @else
                ...
                <div class="panel">
                    <div class="panel-header">
                        ...
                        <a href="{{ \Illuminate\Support\Facades\Route::has('budgets.index') ? route('budgets.index') : '#' }}" class="panel-action">Manage →</a>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:18px;">
                            ...
                            <div>
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                                    ...
                                    <span style="font-size:13px;font-weight:700;color:#1a1a1a;">
                                        ...
                                        <span style="font-weight:400;color:#aaa;">/ ${{ number_format($budget->monthly_limit, 2) }}</span>
                                    </span>
                                </div>
                                <div style="height:6px;background:#f0ece4;border-radius:99px;overflow:hidden;">
                                    <div style="height:100%;border-radius:99px;transition:width 0.6s ease;
                                        width:{{ min(100, $budget->percent_used) }}%;
                                        background-color:{{ $budget->percent_used > 90 ? '#e05c5c' : ($budget->category->color ?? '#FBCF97') }};">
                                    </div>
                                </div>
        ...
        <div class="modal-box" onclick="event.stopPropagation()">
            ...
            <form method="POST" action="{{ route('transactions.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="type-grid">
                        <div class="type-opt active-expense" id="opt-expense" onclick="setType('Expense')">💸 Expense
                        </div>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('keydown', e => {
            if (e.key === 'Escape') closeModal('add-modal');
        });
