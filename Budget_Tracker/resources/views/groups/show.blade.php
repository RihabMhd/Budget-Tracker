...
@push('styles')
    ...
    <style>
        ...
        .balance-card.green .balance-value {
            color: #2EB872;
        }

        .balance-card.red .balance-value {
            color: #e05c5c;
        }
        ...
        .balance-card.green .debt-amount {
            color: #2EB872;
        }

        .balance-card.red .debt-amount {
            color: #e05c5c;
        }
                ...
                @if (count($i_owe) > 0)
                    @foreach ($i_owe as $debt)
                        <div class="debt-row">
                            ...
                            <div class="debt-name">{{ $debt['user']->username }}</div>
                            <div class="debt-amount">{{ number_format($debt['amount'], 2) }} MAD</div>
                            <form method="POST" action="{{ route('groups.members.settle', [$group, $debt['user']]) }}"
                    ...
                    <div class="member-row">
                        ...
                        <div style="flex:1;min-width:0;">
                            <div class="member-name">
                                {{ $member->username }}
                                @if ($isSelf)
                                    <span style="font-size:11px;color:#bbb;">(you)</span>
                                @endif
                            </div>
                        ...
                        @if ($isOwner && !$isSelf)
                            <div class="member-actions">
                                {{-- Transfer ownership --}}
                                <form method="POST" action="{{ route('groups.members.transfer', [$group, $member]) }}"
                        ...
                        <tbody>
                            @foreach ($recent_transactions as $tx)
                                @php
                                    ...
                                    $isSettlement = in_array($tx->type ?? 'expense', [
                                        'settlement_paid',
                                        'settlement_received',
                                    ]);
                                ...
                                <tr class="{{ $isSettlement ? 'settlement-row' : '' }}">
                                    ...
                                    <td style="text-align:right;">
                                        ...
                                        @elseif($mySplit)
                                            <span class="tx-share owe">{{ number_format($mySplit->amount_share, 2) }}
                                                MAD</span>
                                        @else
                    ...
                    @if ($recent_transactions->hasPages())
                        <div class="activity-pagination">
                            ...
                            <div class="pagination-links">
                                ...
                                @else
                                    <a href="{{ $recent_transactions->previousPageUrl() }}" class="pagination-arrow"
                                        title="Previous">
                                        <svg viewBox="0 0 24 24">
                                            <polyline points="15 18 9 12 15 6" />
                                        </svg>
                                ...
                                @if ($recent_transactions->hasMorePages())
                                    <a href="{{ $recent_transactions->nextPageUrl() }}" class="pagination-arrow"
                                        title="Next">
                                        <svg viewBox="0 0 24 24">
        ...
        <div class="modal-box">
            ...
            <form action="{{ route('groups.transactions.store', $group) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="description" class="form-input" placeholder="e.g., Grocery Shopping"
                        required>
                </div>
                <div class="form-group">
